...
<Tabs>
  <Tab title="npm (recommended)">
    ...
    To use unch inside Codex as MCP tools plus a `/unch` slash prompt:

    ```bash
    unch codex install
    ```

    Then restart Codex. The setup command uses `codex mcp add` for the MCP server and writes the reusable prompt to `~/.codex/prompts/unch.md`.
