...
jobs:
  searcher:
    ...
    with:
      ...
      skip_publish: ${{ github.event.inputs.skip_publish == 'true' }}
      unch_repository: %s
      unch_ref: %s
    secrets: inherit
`

// DefaultCIWorkflow is the delegating workflow generated by `unch create ci`.
var DefaultCIWorkflow = fmt.Sprintf(
	defaultCIWorkflowTemplate,
	DefaultCIWorkflowRepository,
...
jobs:
  searcher:
    ...
    with:
      ...
      skip_remote_restore: ${{ github.event.inputs.skip_remote_restore == 'true' }}
      skip_publish: ${{ github.event.inputs.skip_publish == 'true' }}
      unch_repository: ${{ github.repository }}
      unch_ref: ${{ github.sha }}
    secrets: inherit
`
  ...
  workflow_call:
    inputs:
      ...
      skip_publish:
        ...
        type: boolean
      unch_repository:
        description: Repository that provides the unch source used to build the CLI
        required: false
        default: uchebnick/unch
        type: string
      unch_ref:
...
jobs:
  index:
    ...
    env:
      ...
      UNCH_REPOSITORY: ${{ inputs.unch_repository }}
      UNCH_REF: ${{ inputs.unch_ref }}
    ...
    steps:
      ...
      - name: Build unch from source
        ...
        run: |
          ...
          echo "::group::Tooling"
          echo "Using ${UNCH_REPOSITORY}@${UNCH_REF}"
          command -v unch
      ...
      - name: Render GitHub summary
        ...
        run: |
          set -euo pipefail
          {
            ...
            echo "- Tool source: <code>${UNCH_REPOSITORY}@${UNCH_REF}</code>"
            echo
            echo "### Manifest"
            echo
            echo '<pre>'
  ...
  publish:
    ...
    runs-on: ubuntu-latest
    needs: index
    if: ${{ needs.index.result == 'success' && !inputs.skip_publish }}
