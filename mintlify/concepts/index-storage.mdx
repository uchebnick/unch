...
`yzma` runtime libraries are cached globally by unch, not inside each repository state directory. Repository `.semsearch` state stays focused on index data and ranking metadata.
